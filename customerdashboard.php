        ...
        try {
            switch ($_POST['action']) {
                case 'update_measurements':
                    ...
                    $response['message'] = 'Measurements updated successfully!';
                    break;
                ...
                case 'remove_style':
                    ...
                    $remove_stmt->execute();
                    $response['success'] = true;
                    $response['message'] = 'Style removed successfully!';
                    break;
                ...
                case 'update_sizes':
                    ...
                    $response['success'] = true;
                    $response['message'] = 'Size preferences updated successfully!';
                    break;
            }
        } catch (Exception $e) {
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Quick Actions Grid - REMOVED "My Rentals" tab */
        .quick-highlights {
            margin: 20px 0;
        }
        ...
        /* Measurements Styles - Fixed overflow */
        ...
        .style-chip {
            ...
            transition: all 0.2s ease;
            max-width: 100%; /* Prevent overflow */
            word-break: break-word; /* Handle long style names */
        }
...
<body>
    ...
    <header class="nav-wrap" role="banner">
        <div class="nav" role="navigation" aria-label="Main navigation">
            ...
            <div class="icons" role="group" aria-label="User actions">
                <!-- User name display -->
                <span class="user-name">Hi, <?php echo esc($user_data['first_name'] ?? 'User'); ?></span>
    ...
    <main>
        ...
        <div class="dashboard-content">
            ...
            <!-- Quick Highlights Section - REMOVED "My Rentals" tab -->
            ...
            <section class="profile-preferences-section">
                ...
                <div class="preferences-grid">
                    ...
                    <div class="preference-card measurements-card">
                        ...
                        <div class="success-message" id="measurementsSuccess"></div>
                        ...
                        <form method="POST" class="measurements-form" id="measurementsForm" style="display:none;">
                            ...
                            <div class="form-actions">
                                ...
                                <button type="submit" class="btn-primary" id="saveMeasurements">
                                    Save Measurements
                                    <div class="loading-spinner" id="measurementsSpinner"></div>
                                </button>
                            </div>
                        </form>
                    ...
                    <div class="preference-card sizes-card">
                        ...
                        <div class="success-message" id="sizesSuccess"></div>
                        ...
                        <form method="POST" id="sizesForm">
                            ...
                            <div class="style-actions">
                                <button type="submit" class="btn-primary" id="saveSizes">
                                    Save Size Preferences
                                    <div class="loading-spinner" id="sizesSpinner"></div>
                                </button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div class="preference-card styles-card">
                        ...
                        <div class="success-message" id="stylesSuccess"></div>
                        ...
                        <!-- Add Custom Style -->
                        <form method="POST" class="add-style-section" id="customStyleForm">
                            ...
                            <div class="add-style-form">
                                ...
                                <button type="submit" class="btn-secondary" id="addCustomStyle">
                                    Add Custom
                                    <div class="loading-spinner" id="customStyleSpinner"></div>
                                </button>
                            </div>
                        </form>
                    </div>
                        ...
                        <form method="POST" class="contact-form" id="contactForm" style="display:none;">
                            ...
                            <div class="form-actions">
                                ...
                                <button type="submit" class="btn-primary" id="saveContact">
                                    Save Contact Info
                                    <div class="loading-spinner" id="contactSpinner"></div>
                                </button>
                            </div>
                        </form>
    ...
    <script>
        ...
        function submitForm(form, successCallback = null) {
            ...
            const successMessage = form.closest('.preference-card').querySelector('.success-message');
        ...
        document.getElementById('measurementsForm').addEventListener('submit', function(e) {
            ...
            submitForm(this, function(data) {
                ...
                document.getElementById('bustDisplay').textContent = document.getElementById('bustInput').value || '-';
                document.getElementById('waistDisplay').textContent = document.getElementById('waistInput').value || '-';
                document.getElementById('hipDisplay').textContent = document.getElementById('hipInput').value || '-';
        ...
        // Sizes form AJAX
        document.getElementById('sizesForm').addEventListener('submit', function(e) {
            e.preventDefault();
            submitForm(this);
        });
        ...
        document.getElementById('contactForm').addEventListener('submit', function(e) {
            ...
            submitForm(this, function(data) {
                ...
                const addressParts = [];
                const addressLine1 = document.getElementById('addressInput').value;
                const addressLine2 = document.getElementById('addressInput2').value;
                ...
                document.getElementById('addressDisplay').textContent = addressParts.length ? addressParts.join(', ') : '-';
        ...
        // Custom style form AJAX
        document.getElementById('customStyleForm').addEventListener('submit', function(e) {
            e.preventDefault();
            submitForm(this, function(data) {
                // Clear the input
                document.querySelector('#customStyleForm input[name="style_name"]').value = '';
                // Reload the page to show the new style
                setTimeout(() => {
        ...
        function toggleStyle(styleId, element) {
            if (element.classList.contains('selected')) {
                ...
                formData.append('action', 'add_predefined_style');
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Reload to show the new style in chips
                        setTimeout(() => {
                            window.location.reload();
                        }, 500);
                    } else {
                        alert(data.message || 'Error adding style');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
        ...
        function removeStyle(styleId, isCustom) {
            if (confirm('Are you sure you want to remove this style?')) {
                ...
                .then(data => {
                    if (data.success) {
                        // Remove the style chip
                        const chip = document.querySelector(`.style-chip[data-style-id="${styleId}"][data-is-custom="${isCustom}"]`);
                        if (chip) chip.remove();
                        
                        // Update the style selection grid
                        const selectionChip = document.querySelector(`.style-selection-chip[data-style-id="${styleId}"]`);
                        if (selectionChip) selectionChip.classList.remove('selected');
                        
                        // Show success message
                        const successMessage = document.getElementById('stylesSuccess');
                        if (successMessage) {
                            successMessage.textContent = data.message;
                            successMessage.style.display = 'block';
                            setTimeout(() => {
                        ...
                        alert(data.message || 'Error removing style');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    alert('Error removing style');
                });
            }
        }
