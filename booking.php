...
if ($unavailable_result && $unavailable_result->num_rows > 0) {
    while ($row = $unavailable_result->fetch_assoc()) {
        ...
        // Add 2 cleaning days after each booking
        $end_with_cleaning = clone $end;
        $end_with_cleaning->modify('+2 days');
        
        $interval = DateInterval::createFromDateString('1 day');
        $period = new DatePeriod($start, $interval, $end_with_cleaning);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($start_date) && !empty($end_date)) {
        // Check if user already has this EXACT SAME product with OVERLAPPING dates in THEIR OWN cart
            ...
            if ($availability_stmt->num_rows > 0) {
                ...
                if ($stmt->execute()) {
                    if ($stmt->affected_rows > 0) {
                        // Redirect directly to cart without showing notification
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Unavailable dates from PHP (includes 2 cleaning days after bookings)
            ...
            // Check if a date is unavailable (from confirmed bookings + 2 cleaning days)
            ...
            function handleDateClick(date) {
                // Check if the selected date is unavailable (confirmed booking + cleaning days)
                ...
                if (isDateUnavailable(day2) || isDateUnavailable(day3)) {
                    alert('The 3-day rental period is not available. Please select another start date.');
                    return;
                }
