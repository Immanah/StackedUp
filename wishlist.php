...
$sql = "SELECT w.wishlist_id, p.product_id, p.name, p.image, p.price, p.description, p.size 
        FROM wishlist w 
...
// Function to check if product has any available stock
function isProductAvailable($size_str) {
    if (empty($size_str)) {
        return false;
    }
    
    $pairs = explode(',', $size_str);
    foreach ($pairs as $pair) {
        $parts = explode(':', $pair);
        if (count($parts) === 2) {
            $stock = intval($parts[1]);
            if ($stock > 0) {
                return true; // At least one size has stock
            }
        }
    }
    return false; // No sizes have stock
}
?>
            ...
            <div class="wishlist-grid" id="wishlistGrid">
                ...
                <?php else: ?>
                    <?php foreach ($wishlist_items as $item): 
                        $is_available = isProductAvailable($item['size']);
                    ?>
